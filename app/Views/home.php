...
<body>
                    ...
                    <?php foreach ($products as $product): ?>
                        <div class="col-md-3">
                            <div class="product-frame">
                                <form action="<?= base_url('/add/' . $product['product_id']); ?>" method="POST">
                                    <input type="hidden" name="product_id" value="<?php echo $product['product_id']; ?>">
                                    <?php if (!isset($loggedin) || !$loggedin): ?>
                                        <strong>Login</strong>
                                    <?php else: ?>
                                        <input type="hidden" name="loggedin" value="<?php echo $loggedin; ?>">  
                                    <?php endif; ?>
                                    <img src="<?php echo $product['product_image']; ?>" alt="<?php echo $product['product_name']; ?>" class="product-image">
                                    <div class="product-name"><?php echo $product['product_name']; ?></div>
                                    <div class="product-price"><strong>Rp <?php echo $product['product_price']; ?></strong></div>
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function () {
            // Check if the notification query parameter is present in the URL
            const urlParams = new URLSearchParams(window.location.search);
            const notification = urlParams.get('notification');

            // If the notification is present, show it
            if (notification) {
                showNotification(decodeURIComponent(notification));

                // Clear the notification query parameter from the URL to avoid showing it again on page refresh
